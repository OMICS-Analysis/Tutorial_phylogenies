...
tree <- read.newick("results/Example1_all_root.nwk")
...
p <- ggtree(tree_cm, ladderize = TRUE, layout = "circular") + 
  geom_tiplab(aes(label = Strain, angle = angle),
              linesize = .07, size = 4, align = TRUE, offset = 0.008) +
...
ggsave(
  filename = "results/Cmi_all_root/Cmi_all_root_states.png",
  plot = pphy,
  dpi = 300,
  width = 410,   # Ancho en mm
  height = 280,  # Alto en mm
  units = "mm"
)
...
# # Guardar el gráfico en PNG
# ggsave(
#   filename = "results/Cmi_root_d0001/Cmi_root_d0001_AssemblyLevel.png",
#   plot = pphy_2,
#   dpi = 300,
#   width = 360,   # Ancho en mm
...
ggsave(
  filename = "results/Cmi_all_root/Cmi_all_root_verticalTree.png",
  plot = t1,
  dpi = 300,
